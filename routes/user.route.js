...
var transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "hadhrifahd@gmail.com",
    pass: "vbupkrtqpxwqfmyz",
  },
  tls: {
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(404)
        .send({ success: false, message: "Allfields are required" });
    }
    let user = await User.findOne({ email })
      .select("+password")
      .select("+isActive");
      ...
      if (isCorrectPassword) {
        ...
        if (!user.isActive)
          return res.status(400).send({
            success: false,
            message:
              "Your account is inactive, Please contact youradministrator",
          });
...
const generateAccessToken = (user) => {
  return jwt.sign({ iduser: user._id, role: user.role ,isActive:user.isActive }, process.env.SECRET, {
    expiresIn: "6000s",
  });
};
// Refresh
